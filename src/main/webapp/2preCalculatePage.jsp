...
<%
	List<calculateVO> preCalculate = (List) request.getAttribute("preCalculate");
...
<body style="margin: 50px 300px">
	...
	<div>
		<table style="font-family: 'Nanum Brush Script', cursive;">
			...
			<%
				for (calculateVO pre : preCalculate) {
			%>
<tr>
			<td><%=pre.getOrderNum()%></td>
			<td><%=pre.getOrderTime() %></td>
			<td><%=pre.getId() %></td>
			<td><%=pre.getOrderTotalPrice() %></td>
</tr>

			<%
				all=all+pre.getOrderTotalPrice();
				}
			%><tr>
